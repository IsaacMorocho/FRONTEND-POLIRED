...
const RedesPanelAdmin = () => {
  ...
  const [modalEliminar, setModalEliminar] = useState({ visible: false, red: null });
  ...
  const fetchRedes = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.msg || "Error al cargar las redes comunitarias");
    } finally {
      setLoading(false);
    }
  };

  const abrirConfirmacionEliminar = (red) => {
    setModalEliminar({ visible: true, red });
  };

  const cerrarConfirmacionEliminar = () => {
    if (!eliminando) setModalEliminar({ visible: false, red: null });
  };

  const handleEliminarRed = async () => {
    if (!modalEliminar.red?._id) {
      toast.error("No se pudo identificar la red a eliminar");
      return;
    }

    setEliminando(true);
    try {
      await superadminService.deleteRed(modalEliminar.red._id);
      toast.success("Red comunitaria eliminada correctamente");
      setRedes((prev) => prev.filter((red) => red._id !== modalEliminar.red._id));
      setModalVer((prev) => prev.red?._id === modalEliminar.red._id ? { visible: false, red: null } : prev);
      setModalAdmin({ visible: false, admin: null });
      setModalEliminar({ visible: false, red: null });
      await fetchRedes();
    } catch (error) {
      console.error("Error al eliminar la red:", error);
      toast.error(error.response?.data?.msg || "Error al eliminar la red comunitaria");
    } finally {
      setEliminando(false);
    }
  };
  ...
  return (
    ...
    >
                  ...
                  {redesPaginadas.map((red, idx) => (
                    ...
                    >
                      <div className="p-6 flex flex-col items-center relative flex-1">
                        <button
                          onClick={(e) => { e.stopPropagation(); abrirConfirmacionEliminar(red); }}
                        ...
                        <div className="mt-auto pt-4 w-full border-t border-slate-700/50 flex flex-col gap-3">
                          ...
                          <div className="mt-2 flex justify-center">
                            {red.estadoAprobacion === 'aprobada' ? (
                              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${red.deshabilitada
                      ...
                      <div className="flex items-center gap-2">
                        {Array.from({ length: totalPaginas || 1 }, (_, i) => i + 1).map(pageNum => (
                          <button
                            ...
                            className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-medium transition ${paginaActual === pageNum
          ...
          >
            ...
            <div className="px-6 pb-4 relative flex-1 flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-end -mt-10 sm:-mt-12 mb-4">
                ...
                <button
                  onClick={() => abrirConfirmacionEliminar(modalVer.red)}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-600/90 hover:bg-red-500 text-white text-sm font-semibold transition shadow-lg shadow-red-900/20"
                >
                  <MdDelete size={18} /> Eliminar red
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4 justify-center sm:justify-start">
                {modalVer.red.estadoAprobacion === 'aprobada' && (
                  <span className={`px-3 py-1 rounded-full text-xs font-bold shadow-sm ${modalVer.red.deshabilitada ? 'bg-red-900/50 text-red-300 border border-red-800' : 'bg-green-900/50 text-green-300 border border-green-800'
                    }`}>
      ...
      {modalEliminar.visible && modalEliminar.red && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-slate-900 border border-red-900/60 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"
          >
            <div className="p-6 border-b border-slate-800 flex items-start gap-4">
              <div className="w-12 h-12 rounded-full bg-red-950/70 border border-red-800 flex items-center justify-center text-red-300 shrink-0">
                <MdDelete size={24} />
              </div>
              <div>
            ...
            <div className="p-6 border-t border-slate-800 flex flex-col sm:flex-row justify-end gap-3">
              <button
                ...
                disabled={eliminando}
                className="px-4 py-2.5 text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                type="button"
                onClick={handleEliminarRed}
                disabled={eliminando}
                className="px-5 py-2.5 text-sm font-medium bg-red-600 hover:bg-red-500 text-white rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {eliminando ? 'Eliminando...' : <><MdDelete size={18} /> Eliminar definitivamente</>}
              </button>
            </div>
          </motion.div>
        </div>
